...
`request run --output/-o <file>` writes original response bytes to a new file.
Invalid or existing destinations exit `2` before HTTP; completed response
failures still save the bytes and exit `1`. Pre-script/transport failures create
no file, and later write failures preserve diagnostics. Downloads are
unredacted; successful saves report `outputFile`. Binary JSON results contain
metadata rather than body text or bytes. See the
[Automation guide](/docs/guides/automation/#download-a-response) for details.

## Update

Updates noodle to the latest version. Homebrew installs run `brew upgrade noodle` instead. Standalone binaries use Noodle's update manifest and verify the downloaded SHA-256 checksum. Unix replaces the executable atomically; Windows stages the `.exe` for replacement after the running process exits.
...
On Windows, successful staging returns `status: "restart_required"` in JSON and
the TUI shows **Restart to apply**. The completion helper preserves the verified
staged binary and prints its recovery location if replacement fails. Linux musl
is unsupported.
...
Run a dynamic suite by filtering request and inherited folder tags:

```bash
noodle collection run ./api --tag smoke --exclude-tag destructive --fail-fast --delay 500 --json
```

Repeat `--tag` to require every listed tag. Repeat `--exclude-tag` to remove a
request when any listed tag matches. Use `--delay <milliseconds>` to wait
between selected requests; the value must be a non-negative safe integer.

The [Automation guide](/docs/guides/automation/) covers tag suites, selective
request and folder targets, fail-fast and paced runs, response captures and
assertions, JSON results, exit statuses, secrets, and the full collection
workflow. Use `noodle <command> --help` when you need the complete options for
one command.
