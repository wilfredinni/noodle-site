...
```bash
noodle collection run ./api --tag smoke --exclude-tag destructive --fail-fast --json
```

Repeat `--tag` to require every listed tag. Repeat `--exclude-tag` to remove a
request when any listed tag matches.

The [Automation guide](/docs/guides/automation/) covers tag suites, selective
request and folder targets, fail-fast runs, response captures and assertions,
JSON results, exit statuses, secrets, and the full collection workflow. Use
`noodle <command> --help` when you need the complete options for one command.
