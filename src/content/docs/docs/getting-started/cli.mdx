...
Self-updates the binary and verifies the release SHA-256 checksum before
replacing it. Homebrew installs run `brew upgrade noodle` instead.
...
Non-Homebrew installs cache the latest release check for one hour. Set
`GH_TOKEN` or `GITHUB_TOKEN` when the GitHub API needs authentication.
