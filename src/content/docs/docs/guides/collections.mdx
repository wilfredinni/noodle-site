...
[Collection YAML](/docs/reference/collection-format/) reference.

## Starting a collection

For an existing directory open in browse or empty mode, choose
**Initialize Collection** from **Ctrl+P**. To create a new collection directory
from the CLI:
...
Open it with `noodle ./my-api`. Without an explicit path, the TUI opens the
first existing registered collection, falling back to the current directory.
`collection create` also adds a starter request and a `development` environment.
...
children. The [Folders and inheritance](/docs/guides/using-folders/) guide covers the
TUI workflow; the [Collection YAML](/docs/reference/collection-format/)
