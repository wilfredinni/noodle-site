...
never reveal the value. Capture results and RunScope values are not written to
...
Use **Ctrl+G** to toggle the JSON block at the cursor, **F5** to fold every
foldable block, or **F6** to unfold all. You can also click a fold marker in
the line-number gutter. Folded rows keep their original source line numbers,
and copying the response still uses the complete body.
...
When the response pane is focused and the body contains valid JSON, press
