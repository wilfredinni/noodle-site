...
sends run an optional pre-request script, then evaluate every enabled capture
and assertion with a fresh RunScope. When outcomes exist, the tab shows `✓` for
...
details. Script rows show status, duration, log count, normalized errors, and
expandable redacted console messages. Captures show their type and redacted
value; secret-persisted captures and captures from sensitive response headers
never reveal the value. Script source, script results, capture results, and
RunScope values are not written to timeline history. Assertion outcomes are
stored with known secrets redacted, and a successful timeline request snapshot
reflects script mutations to the prepared request.

See [Pre-request Scripting](/docs/guides/pre-request-scripting/) to add a script
