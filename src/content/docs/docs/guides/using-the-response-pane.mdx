---
title: Inspect and save responses
...
Send a request with **Ctrl+Return**, then focus the response pane. Read its
status and timing, select **Body** or **Headers**, and use **Network** when the
connection or authentication needs investigation. **Results** explains failed
checks. For a live binary response, use **Save file** as described below.
...
| **Results** | Script, capture, assertion, and test outcomes from the latest manual send |
...
sends run pre, HTTP, captures, post, assertions, then tests with a fresh RunScope.
...
success, `✗` for a failed script, assertion, capture, or test, or `–` when an earlier
...
See [Scripting](/docs/guides/pre-request-scripting/) to add a script
...
Entries with errors show `ERR` in red. The default retention is 50 entries per
request; change it in [Settings](/docs/guides/settings/#general).
...
Use the response **Cookies** tab to compare cookies sent on the final request
leg with those received. Expand a row with **Return**. The [Cookies guide](/docs/guides/cookies/)
covers jar management, sending rules, storage, and CLI commands.
...
Follow [Troubleshooting](/docs/guides/troubleshooting/) for diagnostic steps.
Use [Response assertions](/docs/guides/assertions/) to turn an expected response
