...
| **Network** | Live request, redirect, response, and failure events |
| **Timeline** | History of every send for this request |

The Network tab updates while a request is sending. Its events are retained in
timeline entries, so you can inspect the trace again after the response is
complete.
...
redact declared secrets plus sensitive request headers and auth. Server response
fields remain intact, so timeline YAML and sidecars should still be treated as
