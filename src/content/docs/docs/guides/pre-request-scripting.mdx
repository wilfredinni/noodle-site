...
Inline scripts and external JavaScript files handle synchronous or async preparation and conditional response
...
Move reusable code into `.js` files and reference them from request YAML,
...
Paths always start at the **collection root**, even from a nested request or
...
A manual send or `request run` loads every applicable pre/post/test source
...
[external-script sample collection](https://github.com/wilfredinni/noodle/tree/main/collections/external-scripts)
...
3. Substitute request variables and [body templates](/docs/guides/using-the-request-pane/#random-and-time-body-values) once.
4. Run pre blocks from collection to request.
5. Send the prepared request.
6. Evaluate captures.
7. Run post blocks from request to collection against the response and committed captures.
