...
Inline scripts handle synchronous preparation and conditional response
...
conditional processing after captures and before assertions.
...
| `noodle.run` | Read, set, or remove RunScope values, with optional persistence |
...
| `noodle.response` | Post only: status, status text, timing, headers, text, and cached JSON |
| `noodle.cookies` | Post only, when available: final-URL-scoped get/set/delete |
...
5. Evaluate captures.
6. Run post against the response and committed captures.
7. Evaluate assertions, including after post failure.
...
response, captures, earlier writes, and logs. Assertions still run. Successful
post values reach later collection requests even when assertions fail.
...
Each invocation has independent Faker 10.5.0 state; a seed resets only its
...
available. See the [time API reference](/docs/reference/collection-format/#script-time-api)
