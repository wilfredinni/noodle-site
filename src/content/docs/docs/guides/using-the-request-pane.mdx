...
When `body_type` is `json`, the body field stays in the [code editor](/docs/guides/code-editor/)
while browsing and editing. It has syntax highlighting, line numbers, code
folding, variable completion, and inline JSON validation. Press **Return** on
the body field to edit it; **Escape** returns to the body-type selector without
discarding the draft. Invalid JSON shows an error notice above the body area
