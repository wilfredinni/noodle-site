...
Set collection names, descriptions, response-history retention, and proxy
policy in `settings.yml`. Agents can use `--noproxy` when a single TUI or
automation run must bypass saved and system proxy settings.
