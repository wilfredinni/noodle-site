...
In an open collection, press **Ctrl+F**, type part of a request name, ID, or URL,
and press **Return** to select a result. The sidebar reveals its folder and the
request pane shows its contents. Press **Ctrl+O** first if the request belongs
to another registered collection.

Continue with [Create and send requests](/docs/guides/using-the-request-pane/)
or use the tree navigation below.
