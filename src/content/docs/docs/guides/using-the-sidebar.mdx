...
Press **Ctrl+F** to search every request in the collection. The finder
matches names, paths, methods, and URLs with fuzzy matching; when an
environment is active, it also matches URLs after `$VARIABLE` references
are resolved. Choose a result with **Return** to reveal and select it.
