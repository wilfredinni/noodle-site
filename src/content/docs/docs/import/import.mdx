...
scripts, and tests are not converted; warnings identify the item and phase.
...
request groups are skipped. Script warnings identify unconverted workspace,
folder, and request hooks without exposing script source.
Supported OAuth 1.0a and OAuth 2.0 fields map to Noodle's matching auth types.

## Script warnings

Postman and Insomnia scripts use foreign APIs and are not converted into Noodle
scripts or tests. CLI output reports each warning; `noodle import ... --json`
includes `data.warnings` with the source format, item path, phase, code, and
message. The TUI summarizes affected items after import. Use **Ctrl+P → Show
Last Notification** to read and scroll the message after it disappears.
Warnings do not prevent supported requests from importing. Review and recreate
required behavior with Noodle's [scripting APIs](/docs/guides/pre-request-scripting/)
before running the imported collection.
