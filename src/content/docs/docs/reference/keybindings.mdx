...
Keybindings can be customized in `~/.config/noodle/keybinds.yml`. Navigation and edit mode keys (`tab`, `up`, `down`, `left`, `right`, `return`, `escape`, `shift+tab`) are fixed and cannot be overridden.

Here is every customizable key with its default:

```yaml
# Request
request_send: ctrl+return
request_save: ctrl+s
request_new: ctrl+n
