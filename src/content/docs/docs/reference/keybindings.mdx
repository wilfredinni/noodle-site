---
title: Keybindings
description: Learn the few navigation patterns that make Noodle fast, then look up any default shortcut when you need it.
---
...
| `Ctrl+Alt+B`         | Copy text response body                              |
| `Ctrl+Alt+S`         | Save live binary response file (Body tab focused)     |
| `Ctrl+Alt+O`         | Open that binary response after saving (Body tab focused) |
| `/`                  | JSONPath in Source or text search in Visual (Body tab focused) |
| `m`                  | Toggle Source / Visual (Body tab focused) |
| `Ctrl+B`             | Toggle the main sidebar (fixed) |
...
The main sidebar can also be toggled with **Toggle Sidebar** in the command
palette. Tab skips it while hidden; **g** then **s** reopens and focuses it.

The command palette includes **Import Collection** and **Export Collection**
alongside request and workspace actions. When the selected request's effective
auth is OAuth 2.0, it also includes actions to fetch or authorize, copy, and
clear the current secure token.
...
![Jump mode labels over the interactive parts of the workspace](../../../../assets/jumpmode.png)
...
The `v` and `c` targets also reveal an empty Assert or Capture tab. Press
`Return` after focusing `+` to open its menu.
...
Binary Save/Open shortcuts can be changed with `response_save_file` and
`response_open_file` in `keybinds.yml` or **F4 → Global → Keyboard**. Open is
available only after that live response has been saved. See
[Images and binary responses](/docs/guides/using-the-response-pane/#images-and-binary-responses).
...
pane_expand: f2
response_copy_body: ctrl+alt+b
response_query: /
response_body_view: m
request_find: ctrl+f
jump_mode: g
