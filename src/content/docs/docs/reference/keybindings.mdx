...
Press `g` to show letter hints on every focusable element. Press a letter
to jump focus to that element, or `Esc` to cancel. Non-matching keys are
swallowed while jump mode is active. Badges render over a dark backdrop
outside the pane layout.
