...
| `tls`             | object  | no       | None     | Per-request TLS override; supports only `verify: true` or `verify: false` |
| `scripts`         | object  | no       | None     | Inline JavaScript or `./file.js` references for `pre` and/or `post`      |
...
| `tests`           | string  | no       | None     | Inline JavaScript or `./file.js` read-only tests after assertions |
...
In JSON, text/XML bodies, and enabled text form values, `$random.method` and
`$time.method` generate values during the single substitution pass before pre
scripts. Optional calls accept JSON literals. Quoted JSON placeholders insert
escaped text; standalone placeholders preserve JSON types. URLs, headers, auth,
form names, file paths, and assertion expectations do not support generation.
Saved YAML retains the templates. See
[random and time body values](/docs/guides/using-the-request-pane/#random-and-time-body-values)
for examples, escaping, limits, and lifecycle rules.

### Sandboxed Pre-request Script

Declare inline or external pre-request and post-response scripts in request YAML, collection
`settings.yml`, or nested `folder.yml`. Start with
[Scripting](/docs/guides/pre-request-scripting/) for a walkthrough.
...
use `./path/to/file.js`, always relative to the collection root. See
[external JavaScript files](/docs/guides/pre-request-scripting/#external-javascript-files)
for source validation and preflight; malformed file references are rejected.
...
Source is never variable-substituted. See [Script API](/docs/reference/script-api/)
for execution order, API methods, persistence, response readers, cookies, limits,
and diagnostics.

#### Script Time API

The date/time method reference is now in [Script API: time](/docs/reference/script-api/#script-time-api).

#### Script Random API

The generator catalog and options are now in [Script API: random](/docs/reference/script-api/#script-random-api).

### Scripted Tests

Optional `tests` accepts an inline JavaScript string or a collection-relative
`./path/to/file.js` reference. Non-string values and malformed paths are rejected.
Empty source is valid; source whitespace survives canonical serialization and is never substituted.
...
Results use `tests: { evaluated, results, logs, error?, errors? }`, separately from pre/post
...
| `scripts` | object | no | Inline or external `pre`/`post` blocks; see phase order below |
| `tests` | string | no | Inline or external tests accumulated in ancestor order |
...
| `scripts` | object | Optional inline or external `pre`/`post` blocks; collection pre runs first and collection post runs last. |
| `tests` | string | Optional inline or external tests that run before descendant test blocks. |
