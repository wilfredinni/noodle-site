...
| `capture`         | map     | no       | None     | Response values for the current automation run             |
...
`--exclude-tag destructive` to remove matches. Exclusion wins when both filters
match.
...
variables. Later requests in the same ordered `collection run` can use those
variables through the normal `$VARNAME` syntax:
...
Variable names must match `^\w+$`. Expressions use `status`, `response.time`,
case-insensitive `headers.<name>`, or `body` followed by dot properties and
array indexes. They are validated when the request loads and are never
variable-substituted.

Each `request run` gets an isolated scope, while one `collection run` shares a
scope across its ordered requests. Environment and secret values load first,
then successful captures override values with the same name. The latest
successful capture wins. Strings substitute verbatim; numbers, booleans, null,
...
Scope values disappear when the command returns. They never modify request
YAML, environment files, collection settings, or timeline history. TUI sends
preserve capture declarations but do not evaluate them.
