...
| `scripts`         | object  | no       | None     | Inline scripts; only string-valued `pre` is supported      |
...
[Pre-request Scripting](/docs/guides/pre-request-scripting/).
...
empty string is a valid no-op. Canonical YAML keeps the literal source before
`capture` and `assert`:
...
Script source is literal and never receives `$VARNAME` substitution. It runs
synchronously after folder overrides and one substitution pass, but before
HTTP. Request and RunScope changes are staged and discarded when the script
fails. On complete success, request mutations apply only to the prepared
in-memory request and RunScope changes commit before HTTP. Later requests in the
same collection run can use those values even when transport, HTTP, capture, or
assertion handling subsequently fails. Manual sends and `request run` use fresh
scopes.

| Global | Members |
| ------ | ------- |
| `request` | Read/write `url` and `method` |
| `request.headers` | `get`, `has`, `set`, `delete` |
| `request.params` | `get`, `getAll`, `set`, `append`, `delete` |
| `request.body` | `text`, `json`, `setText`, `setJson`, `clear` |
| `request.auth` | `clear`, `setBearer`, `setBasic`, `setApiKey` |
| `env` | `get` |
| `run` | `get`, `set`, `unset` |
| `crypto` | `sha256`, `hmacSha256`, `randomBytes` |
| `console` | `log`, `info`, `warn`, `error` |

Header names are case-insensitive. Parameter names are case-sensitive, and
parameter mutations affect only enabled entries. `body.text()` returns textual
JSON, XML, or raw bodies and returns `null` for absent, multipart, URL-encoded,
or binary bodies. `body.json()` throws when the current body is absent or not
valid JSON. Auth helpers replace the complete prepared auth configuration.
`env.get` reads only the selected environment, while `run` reads and stages the
current transient RunScope. Crypto output encoding is exactly `hex` or
`base64`.

Each invocation uses a fresh QuickJS runtime and context with fixed limits:
...
are unsupported. Values crossing the boundary must be bounded JSON with safe
keys, finite numbers, and plain or null-prototype objects.

Treat collections containing scripts as trusted code. `env.get` can read a
selected-environment secret, and a script can place it in the URL, headers, or
body sent immediately afterward.

Scripted requests return a `scripts` result group. Results includes status,
duration, a normalized error, and expandable redacted console logs. Human CLI
output reports status and log count without printing log text; JSON includes
redacted logs. Script source, status, and logs never enter timeline history,
while a successful manual timeline request snapshot reflects prepared request
mutations.
