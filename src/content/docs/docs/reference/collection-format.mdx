...
Declare inline pre-request and post-response scripts in request YAML, collection
`settings.yml`, or nested `folder.yml`. Start with
[Inline scripting](/docs/guides/pre-request-scripting/) for a walkthrough.
...
| `scripts` | object | no | Inline `pre` and/or `post` blocks, accumulated in ancestor order |
| `tests` | string | no | Inline tests accumulated in ancestor order |
...
| `scripts` | object | Optional inline `pre` and/or `post` blocks that run before descendant blocks in each phase. |
| `tests` | string | Optional inline tests that run before descendant test blocks. |
